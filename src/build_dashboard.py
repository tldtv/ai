...
<style>
  ...
  /* Тёмная тема — переключатель в панели настроек (⚙️ -> "Оформление"),
     ...
     синхронизируется (в отличие от остальных настроек в той же панели).
     По умолчанию — "как в системе": ниже сначала идёт вариант через
     prefers-color-scheme (срабатывает, только если человек НЕ выбирал
     тему явно — проверяется через :not([data-theme="light"]), т.к.
     "тёмная" и так перекроется следующим блоком по себестоимости
     специфичности), а затем — явный выбор через [data-theme="dark"],
     который всегда побеждает независимо от системной темы. Почти весь
     остальной CSS уже написан через переменные (var(--ink) и т.д.), а не
     через прямые цвета, поэтому переопределить нужно только их. */
  @media (prefers-color-scheme: dark) {{
    :root:not([data-theme="light"]) {{
      ...
      --ink-soft: #9aa0a8;
      --border: #33373d;
      --green: #3fbf74;    --green-soft: #17301f; --green-ink: #7be3a4;
  ...
  *{{box-sizing:border-box;}}
  ...
  .card-focus-banner{{display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;background:var(--accent-soft);border:1px solid var(--border);border-radius:12px;padding:12px 16px;margin-bottom:18px;font-size:13.5px;color:var(--ink);}}
  ...
  .range-slider input[type=range]::-webkit-slider-thumb{{-webkit-appearance:none;pointer-events:auto;width:16px;height:16px;border-radius:50%;background:var(--accent);border:2px solid var(--surface);box-shadow:0 0 0 1px var(--accent);cursor:pointer;margin-top:0;}}
  .range-slider input[type=range]::-moz-range-thumb{{pointer-events:auto;width:14px;height:14px;border-radius:50%;background:var(--accent);border:2px solid var(--surface);box-shadow:0 0 0 1px var(--accent);cursor:pointer;}}
  ...
  .misc-list{{list-style:none;margin:8px 0 0;padding:0;font-size:12px;color:var(--ink);}}
  ...
  .dup-list{{list-style:none;margin:8px 0 0;padding:0;font-size:12px;color:var(--ink);}}
  ...
  .dup-meta{{color:var(--ink-soft);}}
  .empty{{color:var(--ink-soft);font-size:13px;grid-column:1/-1;}}
  ...
  .card-collapse .body{{margin:8px 0 0;font-size:12.5px;color:var(--ink);line-height:1.5;}}
  ...
  .sub-status.confirmed{{color:var(--green);font-weight:600;}}
...
<script>
  // Применяем сохранённую тему СРАЗУ, до отрисовки body — иначе страница
  // на долю секунды мигнёт светлой темой перед переключением на тёмную
  // (в localStorage у КАЖДОГО посетителя своё значение, ни с чем не
  // синхронизируется — см. панель настроек, раздел "Оформление").
  (function() {{
    try {{
    ...
    }} catch (e) {{ /* localStorage недоступен (приватный режим и т.п.) — остаёмся на "как в системе" */ }}
  }})();
</script>
</head>
<body>
  ...
  <div class="admin-overlay" id="adminOverlay" hidden>
    <div class="admin-modal">
      ...
      <div class="admin-section-title">Оформление</div>
      <div class="tabs" id="themeTabs">
        <button class="tab" data-theme-choice="light" type="button">☀️ Светлая</button>
        <button class="tab" data-theme-choice="dark" type="button">🌙 Тёмная</button>
        <button class="tab active" data-theme-choice="system" type="button">🖥 Как в системе</button>
      </div>
      <p class="note">Только у вас, в этом браузере на этом устройстве — на то, что видят остальные, не влияет, и сохраняется отдельно от всего ниже (не в Firestore).</p>
      <p class="note">Всё ниже открыто всем, у кого есть ссылка на дашборд — как и голосование. Изменения применяются сразу у всех, без нового запуска пайплайна, и хранятся в Firestore поверх значений по умолчанию из config/parameters.yaml.</p>
...
<script>
  ...
  dateFrom.addEventListener('change', () => {{ state.dateFrom = dateFrom.value; freshBtn.classList.remove('active'); apply(); }});
  dateTo.addEventListener('change', () => {{ state.dateTo = dateTo.value; freshBtn.classList.remove('active'); apply(); }});

  // «Свежак» — быстрый фильтр «за последние 7 дней» по дате обнаружения
  // сигнала (date_found, то же поле, что и у обычных полей дат выше).
  // Повторный клик по уже нажатой кнопке снимает фильтр целиком.
  freshBtn.addEventListener('click', () => {{
    const isActive = freshBtn.classList.contains('active');
    if (isActive) {{
      ...
      dateFrom.value = iso(weekAgo); dateTo.value = '';
  ...
  // ---- Тема дашборда (светлая/тёмная/как в системе) — хранится в
  // localStorage у каждого посетителя отдельно, в Firestore НЕ пишется
  // (в отличие от остального в панели настроек ниже) — переключение
  // применяется сразу, без кнопки "Сохранить". Начальное применение при
  // самой первой загрузке страницы — см. <script> сразу после </style>,
  // до отрисовки body (чтобы не было мигания светлой темой перед тёмной).
  ...
  function syncThemeTabs() {{
    let saved = 'system';
    try {{ saved = localStorage.getItem(THEME_KEY) || 'system'; }} catch (e) {{ /* приватный режим браузера и т.п. */ }}
    themeTabs.querySelectorAll('.tab').forEach(btn => {{
      btn.classList.toggle('active', btn.dataset.themeChoice === saved);
    }});
  }}
  ...
  themeTabs.addEventListener('click', e => {{
    ...
    }} catch (e) {{ /* приватный режим и т.п. — тема применится сейчас, но не запомнится после перезагрузки */ }}
    syncThemeTabs();
  }});
  ...
  function answer(raw) {{
    ...
    return 'Могу подсказать: сколько идей зелёных/жёлтых/красных, какая идея топ по оценке, что в рынке «Классифайд»/«Подработка»/«HR-tech», идеи из группы источников (например «РФ» или «Остальной мир»), идеи с низкой или высокой уверенностью, разбивку оценки конкретной идеи — назовите её словами из заголовка.';
  }}
